...
let limitUpcomingDate = new Date(
...
export const discoverMovieByYear = async (year) => {
  try {
    const response = await fetch(
      `${API_ENDPOINT_3}discover/movie?api_key=${API_KEY}&primary_release_year=${year}`
    );
    if (!response.ok) {
      throw Error(response.statusText);
    }
...
export const getSearchResults = async (query) => {
  if (query.length > 0) {
    try {
      const response = await fetch(
        `${API_ENDPOINT_3}search/multi?api_key=${API_KEY}&query=${query}&region=FR&language=en`
      );
      if (!response.ok) {
        throw Error(response.statusText);
      }
